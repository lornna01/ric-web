...
const MovimientoList = ({mensaje,setMensaje,error,setError}) => {
  ...
  doc.text(`CCC ${info.expediente.departamentoId < 100 ? '0' + info.expediente.departamentoId : info.expediente.departamentoId}-${info.expediente.municipioId<100?'0'+info.expediente.municipioId:info.expediente.municipioId}-${info.expediente.poligono}-${info.expediente.predio}`, 97, 50);
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(info.fechaSalida.replace("T", ' ').substring(0, 10), 65, 120);

    doc.setFont('helvetica', 'bold');
    doc.text("Fecha de limite de devolución:", 10, 130);
    doc.setFont('helvetica', 'normal');
    doc.text(info.fechaEntrega.replace("T", ' ').substring(0, 10), 103, 130);
  ...
  doc.setFont('helvetica', 'bold');
  doc.text("Nombre de quién entrega:", 10, 170);
    doc.setTextColor("#f9f9f9");
    doc.setFont('helvetica', 'normal');
  doc.text(":::::: Pendiente ::::::", 90, 170);
  doc.setTextColor("#000");
  doc.text("__________________________", 90, 170);

  doc.setFont('helvetica', 'bold');
  doc.text("Firma de quién entrega:", 10, 190);
    doc.setTextColor("#f9f9f9");
    doc.setFont('helvetica', 'normal');
  doc.text(":::::: Pendiente ::::::", 88, 190);
  doc.setTextColor("#000");
  doc.text("__________________________", 84, 190);

  doc.setFont('helvetica', 'bold');
  doc.text("Nombre de quién entrega:", 10, 200);
    doc.setTextColor("#f9f9f9");
    doc.setFont('helvetica', 'normal');
  doc.text(":::::: Pendiente ::::::", 90, 200);
  doc.setTextColor("#000");
  doc.text("__________________________", 90, 200);

  doc.setFont('helvetica', 'bold');
  doc.text("Firma de quién entrega:", 10, 210);
    doc.setTextColor("#f9f9f9");
    doc.setFont('helvetica', 'normal');
  doc.text(":::::: Pendiente ::::::", 90, 210);
  doc.setTextColor("#000");
  doc.text("__________________________", 84, 210);
    
    doc.setFont('helvetica', 'bold');
    doc.text("Fecha de Devolución:", 10, 230);
    doc.setFont('helvetica', 'normal');
    if (info.createdAt != info.updatedAt) {
      doc.text(info.updatedAt.replace("T", ' ').substring(0, 10), 78, 230);
    } else {
      doc.setTextColor("#f9f9f9");
    ...
    doc.text(new Date().toISOString().replace("T", ' ').substring(0, 16), 51, 260);
    
    
    doc.text("Generado por el usuario:", 100, 260);
    doc.text(user.user.nombre, 136, 260);
    

    
    doc.save("Salida expediente_"+info.id+"_"+info.expedienteId+new Date().getTime()+".pdf");
    //doc.save("demo.pdf");
  }
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (fechaLimiteEntrega < fechaEntregaReal) {
        ...
        mensaje = `El expediente con el CCC: ${expedienteInfo.departamentoId}-${expedienteInfo.municipioId}-${expedienteInfo.poligono}-${expedienteInfo.predio} fue devuelto al Depto de Archivoo.`;     
            ...
            {loading ?
              ...
              return (
                <tr>
                  ...
                  <span
                      className={`text-${
                        item.updatedAt === item.createdAt && fechaLimiteDate.getTime() < fechaActualDate.getTime() ? "danger" : ""
                      }`}
                    >
                  ...
                  <td>
                    <a title="Descargar reporte/Acta" href="javascript:;" onClick={()=>{
                      ...
                      className={`badge bg-label-${
                        item.updatedAt === item.createdAt ? "warning" : "success"
                      } rounded-pill`}
                    >
