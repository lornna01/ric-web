...
const DepartamentoList = ({ user,departamentoList,setMensaje, getDepartamentos,deoartamento,setDepartamento,setEditing, loading,setLoading,error,setError,deleteDepartamento }) => {
  ...
  /*const activarInactivar = async (rol) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(`${import.meta.env.VITE_API_URL}/roles/${rol.id}`, 
        rolActualizado
      );
  ...
  /*const activarInactivar = async (rol) => {
    // Evitar que el rol ADMINISTRADOR sea inactivado
    if (rol.nombre === "ADMINISTRADOR") {
      toast.danger("El rol ADMINISTRADOR no puede ser inactivado.")
      //setError("El rol ADMINISTRADOR no puede ser inactivado.");
      return;
    }
  ...
  const activarInactivar = async (departamento) => {
    if (departamento.nombre === "Petén") {
      console.warn("No se puede inactivar el  departamento");
      return;  // Bloquear la acción si el rol es ADMINISTRADOR
    }
    ...
    try {
      ...
      const response = await axiosInstance.put(`${import.meta.env.VITE_API_URL}/departamentos/${departamento.id}`, departamentoActualizado);
      
      console.log(departamentoActualizado, response.data);
      
      setLoading(false);
      getDepartamentos();  // Actualizar la lista de roles
    } catch (error) {
      console.error("Error en la operación:", error);
      setLoading(false);
    }
